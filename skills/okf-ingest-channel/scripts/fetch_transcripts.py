...
Adapted from coleam00/cole-medin-knowledge-base for the OKF brain vault.
Outputs raw-transcript pages to raw/youtube/ (OKF brain convention).
...
Usage:
    ...
    uv run fetch_transcripts.py @ChannelHandle --limit 10 --output-dir ./raw/youtube
    uv run fetch_transcripts.py @ChannelHandle --since 1y --output-dir ./raw/youtube
"""
import argparse
import json
import re
...
def parse_since(spec: str) -> str | None:
    """Parse a --since value into a YYYY-MM-DD cutoff date.

    Accepts:
      - Relative: '1y', '2y', '365d', '30d'
      - Absolute: '2025-01-01', '20250101'
    Returns None if no filter.
    """
    if not spec:
        return None

    m = re.fullmatch(r"(\d+)y", spec, re.IGNORECASE)
    if m:
        cutoff = datetime.now(timezone.utc) - timedelta(days=365 * int(m.group(1)))
        return cutoff.strftime("%Y-%m-%d")

    m = re.fullmatch(r"(\d+)d", spec, re.IGNORECASE)
    if m:
    ...
    if re.fullmatch(r"\d{8}", spec):
        return f"{spec[:4]}-{spec[4:6]}-{spec[6:8]}"

    print(f"Unrecognized --since format: {spec}", file=sys.stderr)
    print("Use: '1y', '365d', '2025-01-01', or '20250101'", file=sys.stderr)
    sys.exit(2)
...
def upload_date_str(entry: dict) -> str:
    """Extract 'YYYY-MM-DD' from a flat extraction entry, or '' if missing."""
...
def main():
    ...
    ap.add_argument("--output-dir", default="./raw/youtube", help="Where to write raw/*.md files (default: ./raw/youtube)")
    ap.add_argument("--limit", type=int, default=None, help="Max videos to fetch (default: all)")
    ap.add_argument("--since", default=None,
                    help="Only fetch videos published on or after this date. "
                         "Accepts relative (1y, 2y, 365d, 30d) or absolute (2025-01-01, 20250101). "
                         "Older videos are listed in manifest.json under 'older_videos' for later cherry-picking.")
    ...
    if since_date:
        print(f"Date filter: only videos published on or after {since_date}")
    ...
    # Step 1b: filter by date if --since is set (before fetching subtitles)
    older_entries = []
    if since_date:
        to_fetch = []
        for entry in entries:
            published = upload_date_str(entry)
            if not published:
                # If no date, include it rather than silently exclude
        ...
        print(f"After date filter: {len(to_fetch)} to fetch, {len(older_entries)} older (not fetched)")
        entries = to_fetch

    if not entries:
        print(f"No videos to fetch (all {len(older_entries)} videos are older than {since_date}).")
        # Still write a manifest with older videos listed
        manifest = {
            "channel": url,
            "fetched_at": datetime.now(timezone.utc).strftime("%Y-%m-%d"),
            "since": since_date,
            "total_videos": len(entries) + len(older_entries),
            "transcripts_fetched": 0,
            "skipped": 0,
            "older_videos_count": len(older_entries),
            "videos": [],
            "skipped_videos": [],
            "older_videos": [
                {
...
"""
    ...
    if older_entries:
        print(f"Not fetched (older than {since_date}): {len(older_entries)} videos — listed in manifest.json under 'older_videos'")
