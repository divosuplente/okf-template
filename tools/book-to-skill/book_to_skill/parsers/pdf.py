...
def extract_with_docling(pdf_path: str) -> str | None:
    ...
    Figure extraction is enabled: figures are saved alongside the text
    (figures/) and referenced as ![](figures/picture-N.png) in the markdown.

    Formula enrichment (VLM) is intentionally NOT enabled by default: the
    CodeFormulaV2 model costs ~15+ min and 3GB RAM per 30-page chapter, which
    is impractical for whole books. Formulas stay in the per-chapter PDF slices
    (tools/book-slicer.py) — the study concept references the slice pages.
    """
    try:
        ...
        # Figure extraction: save figure crops so markdown can reference real
        # images (figures/picture-N.png) instead of placeholders. Cheap (no VLM).
        ...
        # Save markdown + referenced figures into a temp dir next to the book.
        import tempfile
        from pathlib import Path

        work_dir = Path(pdf_path).parent / ".book_to_skill"
        work_dir.mkdir(parents=True, exist_ok=True)
        md_path = work_dir / (Path(pdf_path).stem + ".docling.md")
        ...
        return md_text or result.document.export_to_markdown()
    except ImportError:
        return None
    except Exception as e:
